defmodule PoolWatch.Blockchain.BlocksTest do
  ...
  defp get_block() do
    %PoolWatch.Blockchain.Blocks{
      epoch_no: 213,
      fees: 924414,
      forged_at: "2020-08-26T13:52:42Z",
      number: PoolWatch.Utils.generate_random_number(),
  ...
  describe "Blocks Actions Test" do
    test "handle_block/3 with default action gives list of notification_channels" do
      assert [twitter, discord] = Blocks.handle_block(get_pool(), get_block(), :default)
    ...
    test "handle_block/3 also gives user_pool_channels" do
      ...
      assert [discord, twitter] = Blocks.handle_block(get_pool(), get_block(), :pool)
    ...
    test "twitter notification" do
      assert [twitter, _] = Blocks.handle_block(get_pool(), get_block(), :default)
      assert %{body: _body, code: code} = PoolWatch.Notification.Channels.request(twitter)
      assert code == 200
    end

    test "discord Notification" do
      assert [_twitter, discord] = Blocks.handle_block(get_pool(), get_block(), :default)
      assert %{body: _body, code: code} = PoolWatch.Notification.Channels.request(discord)
